...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $token_number = strtoupper(trim($_POST['token_number'] ?? ''));
    
    if (empty($phone)) {
        $error = "Please enter your 10-digit registered mobile number.";
    } elseif (!empty($token_number)) {
        // Search by exact Phone + Token Number
        $stmt = $pdo->prepare("
            ...
            WHERE p.phone = ? AND UPPER(qt.token_number) = ? 
            AND (qt.arrival_date = CURRENT_DATE OR DATE(qt.arrival_time) = CURRENT_DATE)
            ORDER BY qt.token_id DESC LIMIT 1
        ");
        $stmt->execute([$phone, $token_number]);
        $search_result = $stmt->fetch();
        
        if (!$search_result) {
            $error = "No active token found for today matching Token '$token_number' and Phone '$phone'.";
        }
    } else {
        // User entered only Phone -> find their latest token today automatically
        $stmt = $pdo->prepare("
            SELECT qt.*, d.name as doctor_name, d.specialization as department, d.room_number, p.name as patient_name 
            FROM queue_tokens qt
            JOIN patients p ON qt.patient_id = p.patient_id
        ...
        if (!$search_result) {
            $error = "No active check-in found for phone number '$phone' today.";
        }
    }
    ...
    if ($search_result && $search_result['status'] === 'Waiting') {
        ...
        $pos_stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM queue_tokens 
            WHERE doctor_id = ? AND status = 'Waiting' AND arrival_time < ? 
            AND (arrival_date = CURRENT_DATE OR DATE(arrival_time) = CURRENT_DATE)
        ");
        $pos_stmt->execute([$search_result['doctor_id'], $search_result['arrival_time']]);
            ...
            <div class="bg-slate-50/80 border border-slate-200 p-4 md:p-5 rounded-xl mb-5 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                ...
                <div class="md:col-span-7 bg-white border border-slate-200/80 rounded-xl p-4 space-y-3 shadow-xs">
                    ...
                    <div class="flex justify-between items-center border-b border-slate-100 pb-2">
                        ...
                        <span class="text-slate-800 font-bold text-xs"><?= htmlspecialchars($search_result['department']) ?></span>
                    </div>
            ...
            <form method="POST" class="space-y-4 text-left">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="tel" id="phone" name="phone" pattern="[0-9]{10}" required class="form-input !pl-10 !py-2.5 !text-sm" placeholder="9876543210" title="10-digit number">
                    </div>
                </div>
                <div>
                    <label for="token_number" class="form-label !text-xs !mb-1.5">Token Number</label>
                    <div class="relative">
                        <i class="ph ph-ticket absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-lg"></i>
                        <input type="text" id="token_number" name="token_number" class="form-input !pl-10 !py-2.5 !text-sm font-bold uppercase" placeholder="e.g. CAR-001 (Optional)">
                    </div>
                </div>
                <div class="pt-2">
